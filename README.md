...
Every row is created with `upsert`, so this is safe to (re-)rusn at any time — including against a database that already has data — it only fills in what's missing, never duplicates or overwrites unrelated rows. Run it once after your first `migrate deploy`, and again after pulling any change to `prisma/seed.ts`.
